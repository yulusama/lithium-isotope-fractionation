...
def solve_series(ages, dLi, volcanic_func, params_dict):
    ...
    for t, dLi_val in zip(ages_desc, dLi_desc):
        ...
        T = params_dict['T_osc']
        if t >= 244.0:
            osc = 1 + beta * np.sin(2 * np.pi * (t - t0_osc) / T)
        else:
            delta_244 = 244.0 - t0_osc
            arg_244 = 2 * np.pi * delta_244 / T
            f0 = np.sin(arg_244)
            f1 = (2 * np.pi / T) * np.cos(arg_244)
            f2 = -(2 * np.pi / T)**2 * np.sin(arg_244)
            dt = t - 244.0
            poly = f0 + f1 * dt + 0.5 * f2 * dt**2
            osc = 1 + beta * poly
...
plt.savefig('Nature_style_panel_final.png', dpi=300, bbox_inches='tight')
plt.show()
...
print("Figure saved as Nature_style_panel_final.png")
